...
console.log("11) Live activity config — generator engine, admin-editable:");
{
  ...
  ok("seven activity types incl. ugc + competition + booster", LIVE_ACTIVITY_TYPES.length === 7 && ["ugc", "competition", "booster"].every((t) => LIVE_ACTIVITY_TYPES.includes(t)) && !LIVE_ACTIVITY_TYPES.includes("onlineChange"));
  ok("default probabilities 38/20/14/12/6/4/6", (() => { const p = d.probabilities; return p.newOrder === 38 && p.delivered === 20 && p.commission === 14 && p.ugc === 12 && p.newAffiliate === 6 && p.competition === 4 && p.booster === 6; })());
  ok("admin probabilities respected", normalizeLiveActivity({ probabilities: { ugc: 90 } }).probabilities.ugc === 90);
  ok("all-zero probabilities fall back to defaults", (() => { const zero = Object.fromEntries(LIVE_ACTIVITY_TYPES.map((t) => [t, 0])); const p = normalizeLiveActivity({ probabilities: zero }).probabilities; return p.newOrder === 38; })());
